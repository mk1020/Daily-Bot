...
const App = () => {
	...
	return (
		<div className="App">
			<AuthComponent>
				<Route exact path="/signin" render={() => isAuthenticated() ? <Dashboard/>:<SignIn />} />
				<Route exact path="/signup" render={() =>isAuthenticated() ? <Dashboard/>: <SignUp />} />
				<PrivateRoute exact path="/" component={Dashboard} isAuthenticated={isAuthenticated()}/>
				<Route path="/forgotpassword" render={() =>isAuthenticated() ? <Dashboard/>: <ForgotPass />} />
			</AuthComponent>
		</div>
	);
};
