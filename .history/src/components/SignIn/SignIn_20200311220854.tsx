...
export default function SignIn() {
	...
	//const [onClickForgotPass, changeOnClickForgotPass] = useState(false);
	let emailForRecovery: string;
	useEffect(() => {
		if (onClickSignIn)
	...
	useEffect(() => {
		if (onClickForgotPass)
			forgotPassword(onClickForgotPass).then(
				success => {
					debugger;
					...
					<Grid container>
						<Grid item xs>
							<Link
								...
								variant="body2"
								onClick={() => {
									emailForRecovery = prompt('Enter your email ', '');
								}}
							>
								Forgot password?
							</Link>
