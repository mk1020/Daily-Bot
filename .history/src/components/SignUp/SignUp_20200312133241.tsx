...
const SignUp = (props: any) => {
	...
	useEffect(() => {
		if (onClickSignUp) {
		if (firstName=='') setIncorrectFirstName("This field cannot be null!")
		if (email=="") setIncorrectEmail("This field cannot be null!");
		if (email.indexOf('@')==-1) setIncorrectEmail("Email entered incorrectly!");
		if (pass=="") setIncorrectPass("This field cannot be null!");
		signUpUser(email, firstName, pass).then((success)=> {
			alert( "Уou have successfully registered!");	
		}, (error)=>{setRegErrorMessage(error.message)})
